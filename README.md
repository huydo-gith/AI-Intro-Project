...
Ứng dụng web demo mô phỏng một **Hệ chuyên gia hỗ trợ chẩn đoán y tế dựa trên luật (Rule-Based Expert System)**. Dự án kết hợp bộ suy diễn logic mệnh đề viết bằng Python ở backend và giao diện tương tác hiện đại ở frontend.
...
4. **Giao diện người dùng (User Interface):**
   * Giao diện web single-page trực quan, tối ưu hóa trải nghiệm người dùng với CSS hiện đại, tập trung vào khung hội thoại chẩn đoán thông minh, hiển thị bộ nhớ làm việc động, sơ đồ vết suy luận và các hộp thoại giải thích trực quan.
...
* **Giải thích lập luận bệnh lý (Diagnosis Explanation):**
  * Tích hợp cơ chế giải thích lập luận chi tiết cho từng kết luận bệnh lý. Người dùng chỉ cần nhấp vào nút **"Giải thích lập luận bệnh"** dưới mỗi thẻ bệnh để hiển thị sơ đồ cây truy vết ngược (Backtracking reasoning chain) từ triệu chứng gốc ban đầu đi qua các luật trung gian để dẫn tới bệnh đó.
* **Giải thích vết suy diễn (Explanation Mechanism):**
  ...
  * Giải thích chi tiết cơ sở khoa học y tế đằng sau mỗi bước suy luận để tăng tính minh bạch của hệ chuyên gia (hộp đen logic được mở ra rõ ràng khi nhấp chuột vào từng luật).
* **Nhận diện ngôn ngữ tự nhiên cơ bản (NLP Parsing):**
  * Sử dụng cơ chế normalize tiếng Việt (xóa dấu, chuẩn hóa chữ thường) và đối khớp từ đồng nghĩa (`synonyms`) được cấu hình sẵn trong KB. Người dùng có thể chat câu tự nhiên ("tôi bị sot, ho khan va dau khop"), hệ thống tự động trích xuất chính xác các mã triệu chứng tương ứng.
...
├── medical_expert/
│   ├── __init__.py
│   ├── knowledge_base.json      # Cơ sở dữ liệu tri thức tĩnh (Facts, Rules, Metadata)
│   ├── knowledge_base.py        # Module quản lý việc đọc cơ sở tri thức vào bộ nhớ
...
├── src/
│   └── app.js                   # Logic giao diện frontend (gọi API, render chat, render trace và modal giải thích)
...
├── app.py                       # HTTP server Python thuần túy cung cấp các REST API cho frontend
...
Dữ liệu được sử dụng bao gồm các bài viết của các trung tâm y tế lớn và uy tín: VNVC, Nhà thuốc Long Châu, Vinmec, Medlatec, ...
